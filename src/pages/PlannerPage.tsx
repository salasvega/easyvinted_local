        ...
        <PageSection>
          ...
          {loading ? (
            ...
            <div className="space-y-6">
                    ...
                    {pendingSuggestions.length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-80 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-emerald-200 scrollbar-track-emerald-50">
                        ...
                        {pendingSuggestions.length > 5 && (
                          <button
                            onClick={() => {
                              const suggestionSection = document.getElementById('suggestions-detail');
                              suggestionSection?.scrollIntoView({ behavior: 'smooth' });
                            }}
                            className="w-full mt-2 text-xs text-emerald-700 hover:text-emerald-800 font-medium py-1.5"
                          >
                            Voir toutes les suggestions ({pendingSuggestions.length})
                          </button>
                        )}
                      </div>
                    )}
              ...
              {/* Bloc détaillé des suggestions IA */}
              {pendingSuggestions.length > 0 && (
                <Card id="suggestions-detail" className="overflow-hidden p-0">
                  <div className="bg-gradient-to-r from-emerald-500 to-teal-600 px-6 py-4 flex items-center gap-3">
                    <Sparkles className="w-6 h-6 text-white" />
                    <h2 className="text-sm sm:text-base font-semibold text-white">
                      Toutes les suggestions de planification
                    </h2>
